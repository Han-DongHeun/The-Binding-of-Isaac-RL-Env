...
def menu(screen:Surface, sounds):
	char = 1
	controls = [K_a, K_d, K_w, K_s, K_LEFT, K_RIGHT, K_UP, K_DOWN, K_q, K_e]
	if HUMAN_MODE:
		return (char, controls, generateSeed())
		...
		for e in event.get():
			...
			if menu == "main":
				...
				if e.key == K_SPACE:
					...
					controldisplay[optionpoint] = key.name(e.key).title()
			...
			if menu == "selection" and e.type == KEYDOWN and e.key == K_ESCAPE:
				menu = "file"
			...
			if menu == "seed" and e.type == KEYDOWN and e.key == K_ESCAPE:     
				menu = "character"
				sounds["pageTurn"].stop()
				sounds["pageTurn"].play()
				for x in range(0,960,70):
			...
			if menu == "character" and e.type == KEYDOWN and e.key == K_ESCAPE:     
				menu = "selection"
				sounds["pageTurn"].stop()
				sounds["pageTurn"].play()
			...
			if menu == "character" and e.type == KEYDOWN and e.key == K_TAB:     
				menu = "seed"
				sounds["pageTurn"].stop()
				sounds["pageTurn"].play()
				for x in range(0,960,70):
			...
			elif menu == "main" and e.type == KEYDOWN and e.key == K_SPACE:
				menu = "file"
				sounds["pageTurn"].stop()
				sounds["pageTurn"].play()
				for x in range(0,540,40):
			...
			elif  menu == "file" and e.type == KEYDOWN and e.key == K_ESCAPE:
				menu = "main"
				sounds["pageTurn"].stop()
				sounds["pageTurn"].play()
				for x in range(0,540,40):
			...
			elif menu == "file" and e.type == KEYDOWN and e.key == K_SPACE and space == 1:
				menu = "selection"
				sounds["pageTurn"].stop()
				sounds["pageTurn"].play()
				for x in range(0,960,70):
			...
			elif menu == "selection" and e.type == KEYDOWN and e.key == K_SPACE and arrowpoint == 0:
				menu = "character"
				sounds["pageTurn"].stop()
				sounds["pageTurn"].play()
				char = 1
				for x in range(0,540,40):
					...
					display.flip()
				continue
			elif menu == "selection" and e.type == KEYDOWN and e.key == K_SPACE and arrowpoint == 1:
				# Continue
				try:
			...
			elif menu == "selection" and e.type == KEYDOWN and e.key == K_SPACE and arrowpoint == 2:
				menu = "options"
				sounds["pageTurn"].stop()
				sounds["pageTurn"].play()
				for x in range(0,960,70):
			...
			elif menu == "options" and e.type == KEYDOWN and e.key == K_ESCAPE and arrowpoint == 2:
				menu = "selection"
				sounds["pageTurn"].stop()
				sounds["pageTurn"].play()
				for x in range(0,960,70):
