...
class Enemy:
	...
	def checkHurt(self, character):
		...
		self.cx, self.cy = (character.x - GRIDX)/GRATIO, (character.y-GRIDY)/GRATIO
		...
		if not self.dead:
			...
			for t in character.tears:
				...
				if dist < self.hurtDistance and t.check_collision():
					t.pop(True)
					if self.canHurt:
						self.hurt(t.damage)
		...
		for tear in self.tears:
			...
			if dist/GRATIO <= character.hurtDistance and tear.check_collision():
